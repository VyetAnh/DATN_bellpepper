...
import torch
GRAPH_DIR = "FullDataset/graph/cnn"
...
CNN_MODEL_SAVE = (
    f"FullModel/modelcnn/{MODEL_NAME}.pt"
)

GRAPH_DIR = (
    f"FullDataset/graph/cnn/{MODEL_NAME}"
)
...
def check_dataset() -> bool:
    """Kiểm tra dataset có đủ ảnh không."""
    total = 0
...
def auto_split(source_dir, val_ratio=0.15, test_ratio=0.15):
    """
    Tự động chia train/val từ thư mục gốc.
    source_dir cấu trúc: source_dir/dry/ wet/ healthy/ bacterial_spot/
    """
    import random
    random.seed(42)
...
def build_model(num_classes):
    ...
    return model
def get_transforms():
    """Data augmentation cho train, normalize cho val."""
...
def test_image(image_path: str):
    """
    Test 1 ảnh đầy đủ pipeline: YOLO crop → CNN classify → kết quả.
    Đặt ảnh test bất kỳ ở đâu cũng được, truyền đường dẫn vào.
    """
    import cv2
    import torch
    import torchvision.transforms as T
